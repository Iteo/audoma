...
def _get_resolver_patterns_and_subresolvers(resolver: URLResolver):
    ...
    for p in resolver.url_patterns:
        if isinstance(p, URLResolver):
            ...
            patterns.append(p)
    return patterns, resolvers

def get_endpoint_pattern(endpoint_name: str, urlconf=None) -> str:
    resolver = get_resolver(urlconf)
    patterns, resolvers = _get_resolver_patterns_and_subresolvers(resolver)
    for r in resolvers:
        new_patterns, new_resolvers = _get_resolver_patterns_and_subresolvers(r)
